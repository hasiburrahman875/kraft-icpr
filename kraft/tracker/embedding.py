...
class EmbeddingComputer:
    ...
    def compute_embedding(self, img, bbox, tag):
        ...
        if self.grid_off:
            ...
            for p in results:
                ...
                if p[3] <= p[1] or p[2] <= p[0]:
                    # If the box has zero or negative area, create a black placeholder patch
                    # This prevents cvtColor from crashing and ensures we have an embedding for every detection
                    crop = np.zeros((self.crop_size[1], self.crop_size[0], 3), dtype=np.uint8)
                else:
                    # If the box is valid, crop as normal
    ...
    def initialize_model(self):
        ...
        path = getattr(self.config, "reid_weights", None)
        if not path:
            raise ValueError("Missing required reid_weights in the runtime config")
