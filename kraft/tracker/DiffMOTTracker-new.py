# DiffMOTtracker.py (stabilized: fast + high HOTA; fixed KF None bug)
# - KF+DiffMOT "no mercy" with motion/scale clamps
# - Matching uses fused boxes via tlwh() (no need to clear KF state)
# - Dynamic per-track candidate gating (IoU band + top-K + center gate)
# - Appearance cost in Stage-A (respects config.w_assoc_emb)
# - FVPD, duplicate/NMS rescue, center rescue, Stage-2, ReID bridge retained
# - Safe ReID cache handling for partial boxes
...
def center_distance_matrix(trk_tlwh: np.ndarray, det_tlwh: np.ndarray,
    ...
    return (dist / max(diag, 1e-6)).astype(np.float32)
    ...
    for i in unmatched_trk_idx:
        ...
        if velocity_vecs is not None:
            ...
            if n > 1e-6:
                ...
                tlwh[0] += 1.0 * v[0]
                tlwh[1] += 1.0 * v[1]
        pseudo.append(tlwh); owners.append(i)
    return pseudo, owners
...
class STrack(BaseTrack):
    ...
    def kf_update_with_measurement(self):
        ...
        if (not self.kf_inited) or (self.mean is None) or (self.covariance is None):
            ...
            self.mean, self.covariance = self.kf_center.predict(self.mean, self.covariance)
    ...
    def multi_predict_diff(stracks, model, img_w, img_h, lambda_kf=1.2, residual_is_normalized=False):
        ...
        W = float(max(img_w, 1e-6)); H = float(max(img_h, 1e-6))

        # current boxes (XYWH)
        cur_xywh  = np.asarray([st.xywh for st in stracks], dtype=np.float32)

        # previous speed (px/frame) for clamp
        prev_xywh  = np.asarray([ (st.xywh_amemory[-1] if len(st.xywh_amemory) >= 1 else st.xywh) for st in stracks ], dtype=np.float32)
        prev2_xywh = np.asarray([ (st.xywh_amemory[-2] if len(st.xywh_amemory) >= 2 else prev_xywh[i]) for i, st in enumerate(stracks) ], dtype=np.float32)
        ...
        gen = model.generate(conds, sample=1, bestof=True, img_w=img_w, img_h=img_h)
        gen = np.asarray(gen)
        if gen.ndim == 3: 
            residual = gen.mean(0).astype(np.float32)
        elif gen.ndim == 2:
            residual = gen.astype(np.float32)
        else:
...
class diffmottracker(object):
    def __init__(self, config, frame_rate=30):
        ...
        self.max_center_frac = getattr(self.config, "max_center_frac", 0.06)
        self.min_iou_gate   = getattr(self.config, "min_iou_gate", 0.10)  # raised default
        self.w_assoc_emb    = getattr(self.config, "w_assoc_emb", 1.0)   # start conservative

        # Formation / rescue
        self.fila_k           = getattr(self.config, "fila_k", 2)
        self.fila_proxy_frac  = getattr(self.config, "fila_proxy_frac", 0.05)
        self.fila_huber_delta = getattr(self.config, "fila_huber_delta", 0.02)
        self.anti_swap_gain   = getattr(self.config, "anti_swap_gain", 0.12)

        self.dup_iou_thr   = getattr(self.config, "dup_iou_thr", 0.72)
        self.dup_max_share = getattr(self.config, "dup_max_share", 1)

        self.fvpd_ttl  = getattr(self.config, "fvpd_ttl", 6)
        self.fvpd_kmin = getattr(self.config, "fvpd_kmin", 2)

        self.emit_unobserved_max = getattr(self.config, "emit_unobserved_max", 0)

        # Ultra-low support (Stage B′)
        self.support_zone_frac = getattr(self.config, "support_zone_frac", 0.06)
        self.ultra_low_thres   = getattr(self.config, "ultra_low_thres", max(0.02, self.config.low_thres - 0.05))
        self.support_iou_gate  = getattr(self.config, "support_iou_gate", 0.10)
        self.support_bonus     = getattr(self.config, "support_bonus", 0.10)

        # ReID
        self.reid_enabled     = getattr(self.config, "reid_enabled", True)
        self.reid_cos_thr     = getattr(self.config, "reid_cos_thr", 0.35)
        self.reid_time_window = getattr(self.config, "reid_time_window", self.buffer_size // 2)

        # DiffMOT fusion flags
        self.residual_is_normalized = getattr(self.config, "residual_is_normalized", False)  # dataset.py uses pixel residuals
        self.lambda_kf = getattr(self.config, "lambda_kf", 0.905)
    ...
    def update(self, dets_norm, model, frame_id, img_w, img_h, tag, img=None):
        ...
        remain_inds = dets_all[:, 4] > self.det_thresh
        inds_low    = dets_all[:, 4] > self.config.low_thres
        inds_high   = dets_all[:, 4] < self.det_thresh
        inds_second = np.logical_and(inds_low, inds_high)
        inds_ultra  = np.logical_and(dets_all[:, 4] >= self.ultra_low_thres,
                                     dets_all[:, 4] <= self.config.low_thres)
        ...
        trust = (dets[:, 4] - self.det_thresh) / max((1 - self.det_thresh), 1e-6)
        dets_alpha = self.alpha_fixed_emb + (1 - self.alpha_fixed_emb) * (1 - trust)

        detections = [STrack(STrack.tlbr_to_tlwh(t[:4]), t[4], f, 30)
        ...
        trk_tlwh = np.array([st.tlwh for st in strack_pool], dtype=np.float32) if len(strack_pool) else np.zeros((0,4), np.float32)
        det_tlwh = np.array([det.tlwh for det in detections], dtype=np.float32) if len(detections) else np.zeros((0,4), np.float32)
        ...
        fused_cost = base_cost + 1.2 * form_matrix + (self.w_assoc_emb * emb_cost)

        # Fallback: ensure each row has finite candidates
        row_has = np.isfinite(fused_cost).any(axis=1) if fused_cost.size else np.array([], dtype=bool)
        if T and D:
            for i in range(T):
                if not row_has[i]:
                    fused_cost[i, :] = iou_cost_full[i, :]

        matches_A = hungarian_min(fused_cost) if fused_cost.size and min(fused_cost.shape) > 0 else np.empty((0,2), dtype=int)
        if len(matches_A):
            matches_A = anti_swap_refine(fused_cost, matches_A, gain=self.anti_swap_gain, max_pairs=20)

        picked_trk = set(matches_A[:,0].tolist())
        picked_det = set(matches_A[:,1].tolist())
        ...
        if len(remain_trk_idx) and len(remain_det_idx):
            ...
            matches_B_sub = hungarian_min(cd_cost_sub) if min(cd_cost_sub.shape) > 0 else np.empty((0,2), dtype=int)
        ...
        for itracked, idet in matched_indices:
            tr = strack_pool[itracked]; det = detections[idet]; alp = dets_alpha[idet]
            if tr.state == TrackState.Tracked:
                tr.update(det, self.frame_id)
                tr.unobserved = False; tr.miss_streak = 0
        ...
        if len(remain_trk_idx2) > 0 and np.any(inds_ultra):
            dets_ultra = dets_all[inds_ultra]
            det_ultra_tlwh = np.array([STrack.tlbr_to_tlwh(t[:4]) for t in dets_ultra], dtype=np.float32)
            det_ultra_xyxy = np.array([t[:4] for t in dets_ultra], dtype=np.float32)
            ...
            cost = np.where(gate, cd_ultra, 1e6)
            m_ultra = hungarian_min(cost) if cost.size and min(cost.shape) > 0 else np.empty((0,2), dtype=int)
            for li, lj in m_ultra:
                if cost[li, lj] >= 1e6: continue
        ...
        if len(dets_second) > 0:
            detections_second = [STrack(STrack.tlbr_to_tlwh(t[:4]), t[4], buffer_size=30)
        ...
        for li, lj in matches2:
            itracked = r_tracked_idx[li]; det = detections_second[lj]; tr = strack_pool[itracked]
            if tr.state == TrackState.Tracked:
                tr.update(det, self.frame_id)
                tr.unobserved = False; tr.miss_streak = 0
                activated_starcks.append(tr)
        ...
        matched_pairs = { int(t): int(d) for t, d in matched_indices }
        ...
        for tlwh, owner in zip(dup_boxes, dup_owners):
            ...
            st.unobserved = True; st.miss_streak += 1
        ...
        for itracked, idet in matches_u:
            ...
            alp = dets_alpha[orig_det_idx] if len(dets_alpha) > orig_det_idx else 0.95
            unconfirmed[itracked].update(detections_u[idet], self.frame_id)
            unconfirmed[itracked].unobserved = False; unconfirmed[itracked].miss_streak = 0
        ...
        if self.reid_enabled and len(unmatched_detections) and len(self.lost_stracks):
            ...
            for lt_i, lt in enumerate(self.lost_stracks):
                if lt.emb is None: continue
                if self.frame_id - lt.end_frame > self.reid_time_window: continue
                for dj, det in enumerate(cand_dets):
                    if getattr(det, "emb", None) is None and img is not None:
                        ...
                        det.emb = emb[0]
                    if getattr(det, "emb", None) is None: continue
            ...
            if pairs:
                ...
                for lt_i, dj, _ in pairs:
                    if lt_i in used_lost or dj in used_det: continue
                    lt = self.lost_stracks[lt_i]; det = cand_dets[dj]
                    lt.re_activate(det, self.frame_id, new_id=False)
                    lt.unobserved = False; lt.miss_streak = 0
                    activated_starcks.append(lt)
                    used_lost.add(lt_i); used_det.add(dj)
        ...
        for inew in unmatched_detections:
            tr = detections[inew]
            if tr.score < self.det_thresh: continue
        ...
        if iou_matrix.size and (self.frame_id % 10 == 0):
            nz_iou = iou_matrix[iou_matrix>0]; nz_cd  = cd_matrix[cd_matrix>0]
